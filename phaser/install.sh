...
Installs the Phaser 3 game skills from ./skills into local agent skill
directories.
...
install_skills() {
  local target="$1"
  local label="$2"
  local manifest="$target/.phaser-game-skills-managed"
  local source_names=" "

  mkdir -p "$target"
  echo "Installing Phaser game skills for $label -> $target"
