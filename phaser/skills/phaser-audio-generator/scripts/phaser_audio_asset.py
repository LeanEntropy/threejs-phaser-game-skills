#!/usr/bin/env python3
"""Generate and process Phaser game audio assets with ElevenLabs."""
...
def multipart_body(fields: dict[str, Any], files: dict[str, Path]) -> tuple[bytes, str]:
    boundary = f"----phaser-audio-{uuid.uuid4().hex}"
    chunks: list[bytes] = []
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate and process Phaser game audio assets.")
...
def main() -> int:
    ...
    try:
        ...
        eprint(f"phaser_audio_asset.py: {exc}")
        return 1
