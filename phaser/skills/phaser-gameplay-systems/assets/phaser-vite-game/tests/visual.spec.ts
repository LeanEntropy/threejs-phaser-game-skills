...
test('renders a nonblank interactive game canvas', async ({ page }, testInfo) => {
  ...
  await page.waitForFunction(() => (window.__PHASER_GAME_DIAGNOSTICS__?.frame ?? 0) > 10);

  const sample = await sampleCanvas(page);
  expect(sample, JSON.stringify(sample)).toMatchObject({ ok: true });

  const before = await page.evaluate(() => window.__PHASER_GAME_DIAGNOSTICS__?.player.position.y ?? 0);
  ...
  await expect
    .poll(async () => page.evaluate(() => window.__PHASER_GAME_DIAGNOSTICS__?.player.position.y ?? 0))
