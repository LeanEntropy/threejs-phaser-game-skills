#!/usr/bin/env python3
"""Create a Phaser 3 Vite game from the packaged skill scaffold."""
...
def create_game(target: Path, force: bool) -> None:
    ...
    print(f"Created Phaser game scaffold at {target.resolve()}")
    print(f"Next: cd {target} && npm install && npm run dev")


def main(argv: list[str]) -> int:
    parser = argparse.ArgumentParser(description="Create a Vite + TypeScript + Phaser 3 browser game scaffold.")
